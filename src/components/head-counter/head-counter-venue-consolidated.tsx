...
export function HeadCounterVenueConsolidated({ dateYmd, sites, cells }: Props) {
  const perSite = totalsBySite(cells);
    ...
    <div className="space-y-4">
      ...
      <div className="overflow-x-auto rounded-lg border">
        <table className="w-full border-collapse text-xs">
          ...
          <tbody>
            {sites.map((s) => {
              const totals = perSite.get(s.id) ?? emptyDirectionTotals();
              const ins = sumRole(totals.in);
              const outs = sumRole(totals.out);
              const hasAny = ins + outs > 0;
              return (
                <tr key={s.id} className={hasAny ? "" : "text-muted-foreground"}>
