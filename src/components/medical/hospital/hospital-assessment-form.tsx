...
import { submitHospitalAssessment } from "@/lib/actions/referrals";
import {
  submitHospitalAssessmentSchema,
  type SubmitHospitalAssessmentInput,
} from "@/lib/schemas/referrals";

interface HospitalAssessmentFormProps {
  referralId: string;
...
export function HospitalAssessmentForm({
  referralId,
  initialDiagnosis,
  treatmentPlan,
  assessmentNotes,
}: HospitalAssessmentFormProps) {
  const [submitting, setSubmitting] = useState(false);
  const [showVitals, setShowVitals] = useState(false);
  const router = useRouter();

  const form = useForm<SubmitHospitalAssessmentInput>({
    resolver: zodResolver(submitHospitalAssessmentSchema),
    defaultValues: {
      id: referralId,
  ...
  async function onSubmit(values: SubmitHospitalAssessmentInput) {
    setSubmitting(true);
    const result = await submitHospitalAssessment(values);
    setSubmitting(false);
    if (result.error) {
